...
class AgentLogger(BaseLogger):
    ...
    def log_query_success(self, query: str, response: str, processing_time: float = 0.0) -> None:
        ...
        self.info(f"QUERY_SUCCESS: Completed in {processing_time:.2f}s")
        self.debug(f"Response: {response_str}")
    
    def log_query_failure(self, query: str, error: str, processing_time: float = 0.0) -> None:
        """Log failed query processing."""
        self.metrics["failed_responses"] += 1
        self.metrics["total_processing_time"] += processing_time
        self._update_average_processing_time()
        
        self.error(f"QUERY_FAILED: {error} (took {processing_time:.2f}s)")
    
    def log_llm_interaction(self, prompt: str, response: str, model: str = "unknown") -> None:
    ...
    def log_tool_plan(self, tools: List[Dict[str, Any]]) -> None:
    ...
    def _update_average_processing_time(self) -> None:
        ...
        total_queries = self.metrics["successful_responses"] + self.metrics["failed_responses"]
        if total_queries > 0:
