...
from lib.errors.llms.gemini import GeminiError
...
class GeminiStrategy(LLMStrategy):
    ...
    def query(self, prompt: str) -> str:
        try:
            ...
            response = self.apiClient.post(f"/{GEMINI_MODEL}:generateContent", json_data=data)
    ...
    def refine(self, prompt: str) -> ToolPlan:
            ...
            "contents": [
                {
                    "parts": [
                        {
                            "text": self._get_system_prompt(prompt=prompt)
                        }
                    ]
                }
            ]
        ...
        try:
            ...
            raise GeminiError(f"Error refining prompt to obtain tool suggestions: {str(e)}")
