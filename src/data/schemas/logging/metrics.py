...
class AgentMetrics(BaseModel):
    ...
    queries_processed: int = Field(default=0, description="Total number of queries processed")
    successful_responses: int = Field(default=0, description="Number of successful responses")
...
class ToolMetrics(BaseModel):
    ...
    successful_calls: int = Field(default=0, description="Number of successful tool calls")
    failed_calls: int = Field(default=0, description="Number of failed tool calls")
    tool_usage: Dict[str, int] = Field(default_factory=dict, description="Usage count per tool")
    execution_sequence: list[str] = Field(default_factory=list, description="Sequence of tool executions")


class APIMetrics(BaseModel):
    """Metrics for API operations."""
    total_calls: int = Field(default=0, description="Total number of API calls")
    successful_calls: int = Field(default=0, description="Number of successful API calls")
