...
def extract_expression(prompt: str) -> Optional[str]:
    ...
    """
    cleaned_prompt = re.sub(r"^(?:what is|calculate|compute|please|find|the result of)\s*", "", prompt, flags=re.IGNORECASE).strip()
    ...
    m = re.search(r"add\s+([0-9]+(?:\.[0-9]+)?)\s+(?:and|to)\s+([0-9]+(?:\.[0-9]+)?)", cleaned_prompt, re.IGNORECASE)
    if m:
        return f"{m.group(1)} + {m.group(2)}"
...
def extract_knowledge_base_query(prompt: str) -> Optional[str]:
    pat = re.compile(r"(?:who\s+is|tell\s+me\s+about|what\s+is)\s+([A-Za-z\s]+?)[\?\.!]*$", re.IGNORECASE)
...
def extract_currency_conversion(prompt: str):
    m = re.search(r"average of (\d+(?:\.\d+)?) and (\d+(?:\.\d+)?) (USD|EUR) into (USD|EUR)", prompt, re.IGNORECASE)
    if m:
        return (m.group(3).upper(), m.group(4).upper(), (float(m.group(1)) + float(m.group(2))) / 2)
    m = re.search(r"convert (\d+(?:\.\d+)?) (USD|EUR) to (USD|EUR)", prompt, re.IGNORECASE)
    if m:
...
class StubLLMStrategy(LLMStrategy):
    ...
    def refine(self, prompt: str) -> ToolPlan:
        ...
        if expr:
            expr = expr.strip()
            if expr:
                suggestions.append(ToolSuggestion(tool="calculator", args=CalculatorArgs(expr=expr).model_dump()))
                cities = extract_cities(prompt)
        if cities:
            for c in cities:
                suggestions.append(ToolSuggestion(tool="weather", args=WeatherArgs(city=c.title()).model_dump()))
        if q:
            suggestions.append(ToolSuggestion(tool="knowledge_base", args=KnowledgeBaseArgs(query=q).model_dump()))
        ...
        if m_avg_add:
            ...
            suggestions.append(ToolSuggestion(tool="calculator", args=calc_args, depends_on=["weather"]))


        cur = extract_currency_conversion(prompt)
        if cur:
            src, tgt, amount = cur
            suggestions.append(ToolSuggestion(tool="currency_converter", args=CurrencyConverterArgs(
